...
export class Backend implements backendInterface {
    ...
    async submitVerification(username: string, adminKey: string): Promise<{ ok: string } | { err: string }> {
        try {
            return await this.actor.submitVerification(username, adminKey);
        } catch (e) {
    ...
    async getSubmissions(): Promise<Submission[]> {
        try {
            return await this.actor.getSubmissions() as Submission[];
        } catch (e) {
            if (this.processError) return this.processError(e);
            throw e;
        }
